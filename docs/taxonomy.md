...
**Why it is the flagship.** Freqtrade's `lookahead-analysis` has documented false negatives on this pattern (issues #11346 and #12507). Case 21 (`eval/cases/htf_merge_11346`) reproduces #11346, and it separates two claims that are easy to conflate.

*The specific bug was not a bug inside freqtrade.* The reporter's strategy defines no `informative_pairs` method at all. They hand-rolled the higher-timeframe join — `matching_indices = pair_dataframe[pair_dataframe['date'] <= row_time].index` — and so bypassed `merge_informative_pair`, which is the thing that would have prevented it: the helper advances the informative row's stamp before joining — its docstring says it "Moves the date of the informative pair by 1 time interval forward", implemented in `strategy_helper.py` as `date_merge = date + minutes_inf - minutes` (one informative period less one base candle, which lands on the base bar whose decision point is the informative bar's close). We did not take that on reading: `ft_clean.py` is `ft_strategy.py` with the hand-rolled join replaced by the helper and nothing else changed, and it drops the win rate from 95.0% to 50.0% over the same 20 trades. Bypassing the helper is what leaked; restoring it *is* the fix, so blaming the helper gets the causation backwards.

*The tool's blindness to this leak class is structural, and independent of how any one strategy introduces it.* Traced through the installed freqtrade 2026.7 source: `optimize/analysis/lookahead.py:146-148` truncates each re-run to the entry candle plus exactly one base candle (`to_dt = result_row["open_date"] + timedelta(minutes=timeframe_to_minutes(...))`, hardcoded, with no config key); `data/dataprovider.py::historic_ohlcv` (line 295) parses that same truncated timerange and loads the *informative* series against it (line 324); and `data/converter/converter.py:159-160` applies the cut as `df.loc[df["date"] <= timerange.stopdt]` — on the `date` column, which freqtrade's own docstring defines as the candle **open**. So a weekly bar stamped Monday passes a truncation to Wednesday, and passes it **whole**, already carrying Friday's close. The full-history run and the truncated run read an identical value, nothing differs, and the tool reports no bias. No truncation on open-date boundaries can expose a leak living inside a single higher-timeframe candle — whether `merge_informative_pair` was used correctly, misused, or bypassed entirely.

*This is a second false-negative mode, not the one freqtrade documents.* The manual concedes only that untriggered signals go unverified — "Signals that are not triggered will not have been verified. This would lead to a false-negative." That caveat does not cover case 21: its signals fire, 20 trades are analysed, and the tool still reports `has_bias: No`. Measured numbers and the full mechanism are in `eval/cases/htf_merge_11346/PROVENANCE.md`; the head-to-head result is in `docs/CHANGELOG.md`.
